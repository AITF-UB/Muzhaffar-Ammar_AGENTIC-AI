...
# 1. NODES
# ================================================================
def refined_rag_query(state:AgentState) -> AgentState: 
    """Fungsi untuk memperbaiki query pencarian RAG berdasarkan konteks yang diberikan, menggunakan LLM."""
    # last_message = state['messages'][-1].content
    # jenjang, kelas, matpel, elemen, materi, atp_str = _get_teacher_context(state["request_params"])
    # query = f"{matpel} {atp_str} {materi}".strip()

    req = state["request_params"]
    
    prompt = """
Task: Ubah mata pelajaran (mapel) dan topik (materi) berikut menjadi kueri pencarian deklaratif yang optimal untuk database RAG (Retrieval-Augmented Generation).

Instruksi:
1. Hapus semua kata tanya (seperti "apa itu", "bagaimana", "mengapa"), kata hubung yang tidak penting, dan kalimat basa-basi.
2. Ubah topik menjadi sebuah frasa atau pernyataan deklaratif/faktual yang padat informasi, mirip dengan judul bab buku teks atau kalimat definisi ilmiah.
3. Gunakan istilah teknis dan baku yang sesuai dengan mata pelajaran yang diberikan.
4. JANGAN sertakan teks percakapan seperti "Berikut adalah kuerinya". Output HARUS HANYA berupa hasil kueri akhir saja.
5. Output hanya berupa query yang sudah diperbaiki, tanpa penjelasan tambahan atau teks lain.
---

Contoh 1:
Input:
Mata Pelajaran: Biologi
Materi: Proses fotosintesis pada tumbuhan dan bagaimana klorofil menangkap cahaya matahari.

Output:
Mekanisme reaksi terang fotosintesis dan peran pigmen klorofil dalam penyerapan foton cahaya pada tumbuhan.

Contoh 2:
Input:
Mata Pelajaran: Sejarah
Materi: Alasan kenapa terjadi perang diponegoro dan dampaknya buat rakyat.

Output:
Latar belakang penyebab Perang Diponegoro serta dampak sosial ekonomi bagi masyarakat Jawa.
---
...
Output: <query optimal untuk RAG>
"""
    mapel_mapping = {
    ...
    query = f"{mapel_str}, {req.get("materi")}"

    refined_query = llm.invoke([SystemMessage(content=prompt.format(materi=req.get("materi"), mapel=mapel_str)), HumanMessage(content=query)])
    better_question = refined_query.content.strip()
    print(f"question_rewriter: Rephrased question: {better_question}")

    return {"refined_rag_query" : better_question}

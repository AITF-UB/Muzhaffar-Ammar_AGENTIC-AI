...
def _run_pipeline_task(job_id: str, pdf_path: Path, params: PipelineParams) -> None:
    ...
    try:
        cfg = PipelineConfig(
            # Teruskan langsung path PDF yang diupload; tidak perlu folder input
            input_pdf         = pdf_path,
...
app = FastAPI(
    title       = "RAG Pipeline Microservice",
    description = "Microservice untuk pipeline PDF → Qdrant (Docling + VLM + BGE-M3 + SPLADE).",
    version     = "1.0.0",
)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("microservice:app", host="0.0.0.0", port=8000, reload=True)
